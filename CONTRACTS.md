...
Salary Showdown creates two random selections from the local role, location, and experience arrays, then evaluates both via `estimateFor`. A game is capped at 10 rounds and locks at completion. Its in-memory score records correct answers, completed rounds, and rounded accuracy percentage.
