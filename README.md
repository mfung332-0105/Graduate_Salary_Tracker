...
The app also shows a separate live-openings panel after an estimate. It reads selected public employer job boards from Figma (Greenhouse) and Swiftly (Lever), matches their published titles to the selected role, and links directly to the official posting. These live jobs are not used to calculate salary estimates. No credentials are required; if a board is unavailable, the panel explains that rather than changing the estimate.
